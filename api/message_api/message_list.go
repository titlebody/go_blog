package message_api

import (
	"github.com/gin-gonic/gin"
	"go_blog/global"
	"go_blog/model"
	"go_blog/model/res"
	"go_blog/utils/jwts"
	"time"
)

type Message struct {
	SendUserID       uint      `json:"send_user_id"` // 发送人ID
	SendUserNickName string    `json:"send_user_nick_name"`
	SendUserAvatar   string    `json:"send_user_avatar"`
	RevUserID        uint      `json:"rev_user_id"` // 接收人ID
	RevUserNickName  string    `json:"rev_user_nick_name"`
	RevUserAvatar    string    `json:"rev_user_avatar"`
	Content          string    `json:"content"`       // 内容
	CreatedAt        time.Time `json:"created_at"`    // 最新消息时间
	MessageCount     int       `json:"message_count"` // 消息条数
}

type MessageGroup map[uint]*Message

func (MessageApi) MessageListView(c *gin.Context) {
	_claims, _ := c.Get("claims")
	claims := _claims.(*jwts.CustomClaims)

	var messageGroup = MessageGroup{}
	var massageList []model.MessageModel
	var messages []Message
	// 获取最新的消息
	global.DB.Order("created_at asc").
		Find(&massageList, "send_user_id = ? or rev_user_id = ?", claims.UserID, claims.UserID)

	for _, models := range massageList {
		// 判断是一个组的条件
		// send_user_id 和rev_user_id 其中一个
		// 1 2 2 1
		// 1 3 3 1 是一组
		message := Message{
			SendUserID:       models.SendUserID,
			SendUserNickName: models.SendUserNickName,
			SendUserAvatar:   models.SendUserAvatar,
			RevUserID:        models.RevUserID,
			RevUserNickName:  models.RevUserNickName,
			RevUserAvatar:    models.RevUserAvatar,
			Content:          models.Content,
			CreatedAt:        models.CreatedAt,
			MessageCount:     1,
		}
		idNum := models.SendUserID + models.RevUserID
		val, ok := messageGroup[idNum]
		if !ok {
			// 不存在
			messageGroup[idNum] = &message
			continue //退出本次循环
		}
		message.MessageCount = val.MessageCount + 1
		messageGroup[idNum] = &message
	}
	for _, messagee := range messageGroup {
		messages = append(messages, *messagee)
	}
	res.OkWithData(messages, c)
	return

}
